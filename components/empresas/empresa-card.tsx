...
import { MoreVertical, Pencil, Eye, X } from "lucide-react";
...
export default function EmpresaCard({
  ...
  const handleToggleActive = async () => {
    try {
      ...
      alert(
        `No se pudo cambiar el estado de la empresa. Por favor, intente nuevamente.`
      );
    } finally {
      setIsLoading(false);
  ...
  const confirmButtonClass = isActive
    ...
    <div className="bg-white rounded-xl shadow-card border border-gray-100 p-5 transition-all duration-200 hover:shadow-card-hover">
      ...
      <ConfirmationModal
        ...
        message={
          <div>
            <p>
              ¿Está seguro que desea {actionText} la empresa{" "}
              <span className="font-semibold">"{empresa.nombre}"</span>?
            </p>
            {isActive && (
              <p className="mt-2 text-red-600">
                Esta acción puede afectar a los registros asociados a esta
                empresa.
              </p>
            )}
          </div>
