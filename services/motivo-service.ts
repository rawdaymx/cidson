...
export class MotivoService {
  static async getAll(
    ...
    try {
      ...
      try {
        ...
        try {
          const alternativeResponse = await HttpClient.get<MotivosPagination>(
            alternativeUrl,
            params
          );
  ...
  private static processMotivosResponse(response: any): MotivosPagination {
    if (response && response.data) {
      const transformedData = Array.isArray(response.data)
        ? response.data.map((item: MotivoApiResponse) => ({
            id: item.id,
            nombre: item.nombre,
            estado: item.estado,
  ...
  static async getById(id: number): Promise<Motivo | undefined> {
    try {
      ...
      try {
        ...
        const alternativeResponse = await HttpClient.get<{
          data: MotivoApiResponse;
        }>(alternativeUrl);
  ...
  static async create(
    motivo: Omit<Motivo, "id" | "fechaCreacion">
  ): Promise<Motivo> {
    try {
      ...
      try {
        const response = await HttpClient.post<{ data: MotivoApiResponse }>(
          url,
          apiMotivo
        );
        return this.transformMotivoResponse(response.data);
        ...
        const alternativeResponse = await HttpClient.post<{
          data: MotivoApiResponse;
        }>(alternativeUrl, apiMotivo);
        return this.transformMotivoResponse(alternativeResponse.data);
      }
    } catch (error) {
  ...
  static async update(
    ...
    try {
      ...
      if (motivo.estado !== undefined)
        apiMotivo.estado = motivo.estado === "Activa" ? true : false;
      if (motivo.configuracion_id !== undefined)
        apiMotivo.configuracion_id = motivo.configuracion_id;
      if (motivo.area_id !== undefined) apiMotivo.area_id = motivo.area_id;

      try {
        const response = await HttpClient.put<{ data: MotivoApiResponse }>(
          url,
          apiMotivo
        );
        return this.transformMotivoResponse(response.data);
        ...
        const alternativeResponse = await HttpClient.put<{
          data: MotivoApiResponse;
        }>(alternativeUrl, apiMotivo);
        return this.transformMotivoResponse(alternativeResponse.data);
      }
  ...
  static async toggleEstado(id: number): Promise<Motivo | undefined> {
    try {
      ...
      try {
        ...
        const response = await HttpClient.delete<{ data: MotivoApiResponse }>(
          url
        );
        return {
          id: response.data.id,
        ...
        const alternativeResponse = await HttpClient.delete<{
          data: MotivoApiResponse;
        }>(alternativeUrl);
        return {
          id: alternativeResponse.data.id,
          nombre: alternativeResponse.data.nombre,
          estado: alternativeResponse.data.estado,
          fecha_creacion: alternativeResponse.data.fecha_creacion,
          configuracion_id: alternativeResponse.data.configuracion_id,
          area_id: alternativeResponse.data.area_id,
        };
      }
    } catch (error) {
