  ...
  static async create(empresaData: EmpresaCreateData): Promise<Empresa> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || `Error al crear empresa: ${response.statusText}`
        );
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 422 && errorData.errors) {
          const error = new Error(
            errorData.message || "Error de validación"
          ) as any;
        ...
        throw new Error(
          errorData.message ||
            `Error al actualizar empresa: ${response.statusText}`
        );
      }
  ...
  static async toggleActive(id: number): Promise<Empresa | undefined> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Error al cambiar estado de empresa: ${response.statusText}`
        );
      }
